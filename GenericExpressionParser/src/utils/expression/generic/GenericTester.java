package utils.expression.generic;


import expression.generic.GenericTabulator;
import utils.base.*;
import utils.expression.common.*;
import utils.expression.parser.ParserTestSet;
...
public class GenericTester extends Tester {
    ...
    private void test(TestGenerator.Test<Integer, F<?>> test, boolean full) {
        modes.values().forEach(mode -> mode.test(test.expr, test.render(NodeRenderer.FULL), full));
    }
    ...
    private <T> void test(final String mode, final String expression, final IF<T> f, final int x1, final int x2, final int y1, final int y2, final int z1, final int z2) {
        final String context = "mode=%s, x=[%d, %d] y=[%d, %d] z=[%d, %d], expression=%s%n"
                .formatted(mode, x1, x2, y1, y2, z1, z2, expression);
...
//            renderer.add("z", 0, (unit, args) -> (x, y, z) -> z);
        ...
        private void freeze(final Set<String> operations) {
            ...
            final List<String> diff = operations.stream()
                    ...
                    .toList();
            Asserts.assertTrue("Missing operations for %s: %s".formatted(mode, diff), diff.isEmpty());
        }
    }
